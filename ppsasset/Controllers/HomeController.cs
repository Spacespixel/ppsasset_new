...
namespace PPSAsset.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> RegisterProject(RegistrationInputModel input)
        {
            ...
            _logger.LogInformation("=== UTM Debug: Source={Source}, Medium={Medium}, Campaign={Campaign}, Term={Term}, Content={Content} ===", 
                input.UtmSource, input.UtmMedium, input.UtmCampaign, input.UtmTerm, input.UtmContent);
            ...
            try
            {
                ...
                try 
                {
                    _ = Task.Run(async () => {
                        try 
                        {
                             ...
                             {
                                 _logger.LogWarning($"[Email Debug] Skipping email: No ProjectEmail defined for project {input.ProjectID}");
                             }
                             else
                             {
                                 _logger.LogInformation($"[Email Debug] Attempting to send email to {toEmail}");
                                 var subject = $"ข้อมูลการลงทะเบียนของโครงการ {input.ProjectName}";
                                 var body = $@"
                                    <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;'>
                                        <h2 style='color: #333; border-bottom: 2px solid #4A7030; padding-bottom: 10px;'>ข้อมูลว่าที่ลูกค้าใหม่: {input.ProjectName}</h2>
                                        
                                        <table style='width: 100%; border-collapse: collapse; margin-top: 20px;'>
                                            <tr>
                                                <td style='padding: 8px; font-weight: bold; width: 150px; color: #555;'>Name:</td>
                                                <td style='padding: 8px;'>{input.FirstName} {input.LastName}</td>
                                            </tr>
                                            <tr>
                                                <td style='padding: 8px; font-weight: bold; color: #555; background-color: #f9f9f9;'>TelNo:</td>
                                                <td style='padding: 8px; background-color: #f9f9f9;'>{input.TelNo}</td>
                                            </tr>
                                            <tr>
                                                <td style='padding: 8px; font-weight: bold; color: #555;'>Email:</td>
                                                <td style='padding: 8px;'><a href='mailto:{input.Email ?? "-"}' style='color: #4A7030;'>{input.Email ?? "-"}</a></td>
                                            </tr>
                                            <tr>
                                                <td style='padding: 8px; font-weight: bold; color: #555; background-color: #f9f9f9;'>งบประมาณ:</td>
                                                <td style='padding: 8px; background-color: #f9f9f9;'>{input.Budget ?? "-"}</td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td style='padding: 8px;'>{input.AppointmentDate?.ToString("yyyy-MM-dd") ?? "-"} เวลา: {input.AppointmentTime ?? "-"}</td>
                                            </tr>
                                        </table>

                                        <div style='margin-top: 30px; background-color: #f0f4eff; padding: 15px; border-radius: 5px; border-left: 4px solid #4A7030;'>
                                            <h4 style='margin: 0 0 10px 0; color: #4A7030;'>Marketing Source</h4>
                                            <p style='margin: 5px 0; font-size: 14px;'><strong>UTM Source:</strong> {input.UtmSource ?? "-"}</p>
                                            <p style='margin: 5px 0; font-size: 14px;'><strong>UTM Medium:</strong> {input.UtmMedium ?? "-"}</p>
                                            <p style='margin: 5px 0; font-size: 14px;'><strong>UTM Campaign:</strong> {input.UtmCampaign ?? "-"}</p>
                                            <p style='margin: 5px 0; font-size: 14px;'><strong>UTM Term:</strong> {input.UtmTerm ?? "-"}</p>
                                            <p style='margin: 5px 0; font-size: 14px;'><strong>UTM Content:</strong> {input.UtmContent ?? "-"}</p>
                                        </div>
                                        
                                        <div style='margin-top: 20px; font-size: 12px; color: #999; text-align: center; border-top: 1px solid #eee; padding-top: 10px;'>
                                            Sent from PPS Asset Website • {DateTime.Now:dd/MM/yyyy HH:mm}
                                        </div>
                                    </div>
                                 ";
                ...
                {
                    _logger.LogError(ex, "Failed to initiate background email sending");
                }
