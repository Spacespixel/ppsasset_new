    ...
    public class DatabaseProjectService : IProjectService
    {
        ...
        private ProjectViewModel MapToProjectViewModel(dynamic project)
        {
            return new ProjectViewModel
            {
                ...
                Details = new ProjectDetails
                {
                    ...
                    PropertyType = project.ProjectTypeDesc ?? string.Empty
                },
                ProjectEmail = project.ProjectEmail ?? string.Empty
            };
        }
        ...
        private ProjectImages LoadBasicProjectImages(IDbConnection connection, string projectId)
        {
            try
            {
                ...
                // If Hero is still empty, use Thumbnail as fallback
                if (string.IsNullOrEmpty(projectImages.Hero) && !string.IsNullOrEmpty(projectImages.Thumbnail))
                {
                    projectImages.Hero = projectImages.Thumbnail;
                }
